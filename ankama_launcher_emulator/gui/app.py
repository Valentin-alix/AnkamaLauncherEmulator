...
from ankama_launcher_emulator.gui.account_card import build_account_card, make_game_handler
...
def run_gui() -> None:
    ...
    def index():  # pyright: ignore[reportUnusedFunction]
        ...
        for account in accounts:
            ...
            dofus_btn, retro_btn, ip_select, proxy_input = build_account_card(login, all_interface)

            def launch_dofus(source_ip, raw_proxy, _login=login):
                proxy_listener, proxy_url = build_proxy_listener(raw_proxy)
                server.launch_dofus(_login, proxy_listener=proxy_listener, proxy_url=proxy_url, source_ip=source_ip)

            def launch_retro(source_ip, raw_proxy, _login=login):
                server.launch_retro(_login, proxy_url=raw_proxy, source_ip=source_ip)

            dofus_btn.on_click(make_game_handler(login, ip_select, proxy_input, "Dofus 3", launch_dofus))
            retro_btn.on_click(make_game_handler(login, ip_select, proxy_input, "Retro", launch_retro))
